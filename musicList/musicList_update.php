...
// }
        */
        $newPath = './music/' . substr($songPath, 17, 13) . '-' . $_FILES['uploadSong']['name']; //==>保证能保留上一次该元素的随机id
        // /musicList/music/5d0110a33f87e- 17 13
        if (!move_uploaded_file($_FILES['uploadSong']['tmp_name'], $newPath)) {
            echo '<h1>移动音乐失败</h1>';
        }
        // 生成一个新路径
